...
namespace MTRK {
  ...
  */
  ...
  public:
    ...
    std::map<int, int> process(ObservationModelType& om, observ_model_t om_flag = CARTESIAN, association_t alg = NN, unsigned int seqSize = 5, double seqTime = 0.2, double stdLimit = 1.0)
      {
	// data association
  ...
  for (ai = m_assignments.begin(); ai != aiEnd; ai++) {
    assignments.insert(make_pair(ai->first, m_filters[ai->second].id));
  }
