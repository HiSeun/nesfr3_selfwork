...
def load_trimg(folder):
    ...
    for filename in os.listdir(folder):
        ...
        if img is not None:
            ...
            imghue = cv2.calcHist([img],[0, 1], None, [180, 256], [0, 180, 0, 256])
            #images.append(img)
...
def hist_callback(data_img, data_hist):
    ...
    for i, tracking_id in enumerate(data_hist.tracking_id):
        ...
        position = (10,20)
        path = "/mnt/catkin_ws/src/TestResults1223/"
        if tracking_id < 0 :
            pass
            ...
            if id_nums[trackings.index(tracking_id)] != None :
                ...
                cv2.imwrite(os.path.join(path+"/actor"+str(id_nums[trackings.index(tracking_id)]), str(data_hist.header.seq)+".png"), hsv_image)
                cv2.putText(
    ...
    database.id_num = id_nums
    database.tracking_id = trackings
    tracking_pub.publish(database)
...
def id_num(hist_hue) : 
    ...
    for hue_ in hue1 :
        chk = 0
        result1 = cv2.compareHist(hue_, hist_hue, cv2.HISTCMP_CORREL)
        result2 = cv2.compareHist(hue_, hist_hue, cv2.HISTCMP_CHISQR)
        result3 = cv2.compareHist(hue_, hist_hue, cv2.HISTCMP_INTERSECT)
        result4 = cv2.compareHist(hue_, hist_hue, cv2.HISTCMP_BHATTACHARYYA)
    ...
    print("Correspondence 1 : %d / %d, %.2f %%"%(success, len(hue1), (success*100)/len(hue1)))
    corr[0] = success*100/len(hue1)
    success = 0
    ...
    print("Correspondence 2 : %d / %d, %.2f %%"%(success, len(hue2), success*100/len(hue2)))
    corr[1] = success*100/len(hue2)
    success = 0
    ...
    print("Correspondence 3 : %d / %d, %.2f %%"%(success, len(hue3), (success*100)/len(hue3)))
    corr[2] = success*100/len(hue3)
    success = 0
    ...
    print("Correspondence 4 : %d / %d, %.2f %%"%(success, len(hue4), (success*100)/len(hue4)))
    corr[3] = success*100/len(hue4)
    success = 0
    ...
    #         success = success + 1
    #     else : pass
    # print("Correspondence 5 : %d / %d, %.2f %%"%(success, len(hue5), success*100/len(hue5)))
    # corr[4] = success*100/len(hue5)
    # success = 0
    ...
    #         success = success + 1
    #     else : pass
    # print("Correspondence 6 : %d / %d, %.2f %%"%(success, len(hue6), success*100/len(hue6)))
    # corr[5] = success*100/len(hue6)

    result = corr.index(max(corr))+1
    return result
...
if __name__ == "__main__":
    ...
    tracking_pub = rospy.Publisher("/nesfr3/" + robot_index + "/identification", TrackingData, queue_size = 1)
    ...
    hue1 = load_trimg("/mnt/catkin_ws/src/newactor/actor1")
    hue2 = load_trimg("/mnt/catkin_ws/src/newactor/actor2")
    hue3 = load_trimg("/mnt/catkin_ws/src/newactor/actor3")
    hue4 = load_trimg("/mnt/catkin_ws/src/newactor/actor4")
    hue5 = load_trimg("/mnt/catkin_ws/src/newactor/actor5")
    hue6 = load_trimg("/mnt/catkin_ws/src/newactor/actor6")
    rospy.spin()
