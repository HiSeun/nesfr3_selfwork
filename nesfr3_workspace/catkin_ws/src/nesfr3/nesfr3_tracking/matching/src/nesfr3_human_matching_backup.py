...
def hist_callback(data_img, data_hist):
    ...
    for i, tracking_id in enumerate(data_hist.tracking_id):
        ...
        for hue_ in hues :
            result1 = result1 + cv2.compareHist(hist_hue, hue_, cv2.HISTCMP_CORREL)
            result2 = result2 + cv2.compareHist(hist_hue, hue_, cv2.HISTCMP_CHISQR)
            result3 = result3 + cv2.compareHist(hist_hue, hue_, cv2.HISTCMP_INTERSECT)
            result4 = result4 + cv2.compareHist(hist_hue, hue_, cv2.HISTCMP_BHATTACHARYYA)
        ...
        if chk == 4 :
            print("[%d] CORREL : %.3f, CHISQR : %.3f, INTERSECT : %.3f, BHATTACHARYYA : %.3f"%(data_hist.header.seq,result1,result2,result3,result4))
            print("This is actor 1")
            actorID = "1"
        else :
            print("------------------------------------------------------------------------------------------------------------")
            print("This is actor 2")
            actorID = "2"            

        # Plot the histogram
        # histt = np.int32(np.around(hist_hue))
        ...
        position = (int(hist_x/5) , 10)
        cv2.putText(
             box_image_resize, "Actor_"+actorID, position, cv2.FONT_HERSHEY_SIMPLEX, 0.5, (209, 80, 0, 255), 2) #font stroke
        # path = "/mnt/catkin_ws/src/TestResults"
        ...
        h_image = box_image_resize
        print("4")
        cv2.imshow("hist 1D", box_image_resize)
        print("5")
        cv2.waitKey(30) 
        print("6")    
