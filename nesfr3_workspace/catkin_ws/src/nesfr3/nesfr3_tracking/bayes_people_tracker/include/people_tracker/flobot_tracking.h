...
class SimpleTracking {
 public:
  SimpleTracking(double sLimit = 1.5) {
    time = getTime();
  ...
  std::map<int, int> addObservation(std::string detector_name, std::vector<geometry_msgs::Point> obsv, double obsv_time, geometry_msgs::Pose &robot_pose) {
    boost::mutex::scoped_lock lock(mutex);
    ...
    if(det.om_flag == CARTESIAN) {
      m_assignments = mtrk.process(*(det.ctm), det.om_flag, det.alg, det.seqSize, det.seqTime, stdLimit);
    }
    if(det.om_flag == POLAR) {
      //det.plm->update(robot_pose.position.x, robot_pose.position.y, robot_pose.orientation.w);
      det.plm->update(0, 0, 0);
      m_assignments = mtrk.process(*(det.plm), det.om_flag, det.alg, det.seqSize, det.seqTime, stdLimit);
    }
