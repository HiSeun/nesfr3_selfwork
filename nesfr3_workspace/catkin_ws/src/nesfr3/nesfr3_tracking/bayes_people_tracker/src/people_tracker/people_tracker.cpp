...
PeopleTracker::PeopleTracker() : detect_seq(0), marker_seq(0) {
  ros::NodeHandle n(ros::this_node::getNamespace());
  ...
  private_node_handle.param("base_link", base_link, std::string("nesfr3/1/base_link"));
  private_node_handle.param("target_frame", target_frame, std::string("nesfr3/1/base_link"));
  private_node_handle.param("tracker_frequency", tracker_frequency, double(100.0));
  ...
  pub_detect = n.advertise<bayes_people_tracker::PeopleTracker>(ros::this_node::getNamespace() + pub_topic.c_str(), 1, con_cb, con_cb);
  private_node_handle.param("pose", pub_topic_pose, std::string("/people_tracker/pose"));
  pub_pose = n.advertise<geometry_msgs::PoseStamped>(ros::this_node::getNamespace() + pub_topic_pose.c_str(), 1, con_cb, con_cb);
  private_node_handle.param("pose_array", pub_topic_pose_array, std::string("/people_tracker/pose_array"));
  pub_pose_array = n.advertise<geometry_msgs::PoseArray>(ros::this_node::getNamespace() + pub_topic_pose_array.c_str(), 1, con_cb, con_cb);
  private_node_handle.param("people", pub_topic_people, std::string("/people_tracker/people"));
  pub_people = n.advertise<people_msgs::People>(ros::this_node::getNamespace() + pub_topic_people.c_str(), 1, con_cb, con_cb);
  private_node_handle.param("trajectory", pub_topic_trajectory, std::string("/people_tracker/trajectory"));
  pub_trajectory = n.advertise<geometry_msgs::PoseArray>(ros::this_node::getNamespace() + pub_topic_trajectory.c_str(), 1, con_cb, con_cb);
  private_node_handle.param("marker", pub_topic_marker, std::string("/people_tracker/marker_array"));
  pub_marker = n.advertise<visualization_msgs::MarkerArray>(ros::this_node::getNamespace() + pub_topic_marker.c_str(), 1, con_cb, con_cb);
  
  pub_hist = n.advertise<nesfr3_msgs::BlobsArray>(ros::this_node::getNamespace() + "/histogram", 1);
  boost::thread tracking_thread(boost::bind(&PeopleTracker::trackingThread, this));
  
  ros::spin();
}

// int PeopleTracker::findMissingID(std::vector<long> v){
//   int idSum = 0;
//   int missingID = 0;
//   idSum = std::accumulate(v.begin(), v.end(), 0);
//   missingID = v[15];
//   //missingID = 120 + v.back() - idSum;
//   return missingID;
// }
...
void PeopleTracker::trackingThread() {
  ...
  while(ros::ok()) {
	...
	uuids.push_back(std::to_string(it->first));
	pids.push_back(it->first);

  
  /////////////////
  // int idSum = 0;
  // idSum = std::accumulate(pids.begin(), pids.end(), 0);
  
 	// if (pids.back() > 15) {
  ///////////From here/////////////////////
  // int psize, csize;
  // csize = pids.size();
  // if (pids.back() > 15) { // Check whether an actor was lost. (It appends new ID, not the same ID)
  //   std::cout << "Actor lost! Last ID is " << pids.back() << ", Number of Actors is " << pids.size() <<std::endl;
  //   if (csize > 16) {
  //     psize = csize;

  //   }
  //   else if (csize == 16) { // if the counted actor value is the same as the real value 16, 
  //     for (int i=0; i<pids.size(); i++) {
  //       if (i != pids[i]) { // finding the missed number. check whether the number is located in the right place.
  //         missingID = i;
  //         std::cout << "Missing ID is " << missingID << std::endl;
  //         pids.insert(pids.begin()+i, i); // Inserting the missing ID at the right place.
  //         uuids.insert(uuids.begin()+i, std::to_string(missingID));
  //         std::cout << "Insert ID at " << i << std::endl;
  //         pids.erase(pids.begin()+16, pids.end()); // delete the last exceed actor ID. since we have inserted the right ID in the right place.
  //         uuids.erase(uuids.begin()+16, uuids.end());
  //         std::cout << "myvector contains:";
  //         for (unsigned i=0; i<pids.size(); ++i)
  //           std::cout << ' ' << pids[i] << '\n'; // Show the total actor IDs.
  //       } 
  //     }
  //   }
  // }
  ...
  for(int i = 0; i < previous_poses.size(); i++) {
	...
	if(log_trajectories) {
	  for(int k = 0; k < trajectory.poses.size(); k++)
	    std::cerr << trajectory.poses[k].position.x << " " << trajectory.poses[k].position.y << std::endl; //출력
	}
      }
    }
  }
...
void PeopleTracker::createVisualisation(std::vector<geometry_msgs::Pose> poses,
  ...
  for(int i = 0; i < poses.size(); i++) {
    ...
    tracking_id.id = pids[i]; // 3d box 위 id
...
void PeopleTracker::detectorCallback(const nesfr3_msgs::BlobsArray::ConstPtr &pta, std::string detector) {
  ...
  if(ppl.size()) {
    if(ekf == NULL) {
      if(ukf == NULL) {
	m_assignments = pf->addObservation(detector, ppl, pta->header.stamp.toSec(), robotPoseInTargetCoords);
      } else {
	m_assignments = ukf->addObservation(detector, ppl, pta->header.stamp.toSec(), robotPoseInTargetCoords);
      }
    } else {
      m_assignments = ekf->addObservation(detector, ppl, pta->header.stamp.toSec(), robotPoseInTargetCoords);
    }
  }
  typename std::map<int, int>::iterator ai, aiEnd = m_assignments.end();
  nesfr3_msgs::BlobsArray::Ptr hist(new nesfr3_msgs::BlobsArray);
  *hist = *pta;
  for(int i = 0; i < hist->blobsArray.size(); i++) hist->tracking_id[i] = -1;
  for (ai = m_assignments.begin(); ai != aiEnd; ai++) {
    hist->tracking_id[ai->first] = int(ai->second);
    pub_hist.publish(hist);
  }
}
...
void PeopleTracker::connectCallback(ros::NodeHandle &n) {
  ...
  if(!loc && !pose && !pose_array && !people && !trajectory && !markers) {
    ...
    for(it = subscribers.begin(); it != subscribers.end(); ++it)
      subscribers[it->first] = n.subscribe<nesfr3_msgs::BlobsArray>(ros::this_node::getNamespace() + "/blobsArray", 1, boost::bind(&PeopleTracker::detectorCallback, this, _1, it->first.first));
  }
}
