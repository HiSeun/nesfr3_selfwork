...
class BBoxVisualization():
    ...
    def draw_bboxes(self, img, boxes, cl):
        """Draw detected bounding boxes on the original image."""
        for bb in boxes:
          x_min, y_min, x_max, y_max = bb.xmin, bb.ymin, bb.xmax, bb.ymax
          color = self.colors[cl]

          if x_min > x_max :
            cv2.rectangle(img, (x_min, y_min), (4400, y_max), color ,2)
            cv2.rectangle(img, (0, y_min), (x_max, y_max), color ,2)
          else :
            cv2.rectangle(img, (x_min, y_min), (x_max, y_max), color, 2)
          txt_loc = (max(x_min+2, 0), max(y_min+2, 0))
          cls_name = self.cls_dict.get(cl, 'CLS{}'.format(cl))
          txt = '{} {:.2f}'.format(cls_name, bb.score)
