...
def allocate_buffers(engine):
    ...
    for binding in engine:
        ...
        logging.info('cuda mem_alloc start')
        device_mem = cuda.mem_alloc(size * dtype.itemsize)
        logging.info('cuda mem_alloc end')
...
class EngineRunner():
    ...
    def __init__(self, engine_file, verbose=False, plugins=None):
        ...
        logging.info('engine create_execution_context')
        self.context = self.engine.create_execution_context()
        logging.info('engine create_execution_context end')
       # cuda.Context.attach()
    ...
    def __call__(self, inputs, batch_size=1):
        ...
        print('cuda memcpy_dtoh_async')
        [cuda.memcpy_dtoh_async(out.host, out.device, self.stream) for out in self.outputs]
        # Synchronize the stream
        print('cuda stream sync')
